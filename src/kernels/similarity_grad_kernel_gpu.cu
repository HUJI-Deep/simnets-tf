...
template<typename T>
class SimilarityParametersGradKernelGPU : public SimilarityKernelCommon {
public:

    using Base = SimilarityKernelCommon;
    using Dtype = T;

    SimilarityParametersGradKernelGPU(OpKernelConstruction *context) : Base(context) {}
    ...
    void Compute(OpKernelContext *context) override {
        ...
        interlaced_grad_t.device(context->eigen_gpu_device()) = interlaced_grad_t.constant(0);
        ...
        interlace_gpu<Dtype>(M_ * K_,   templates_t.data(), weights_t.data(), inter_params);
        ...
        for (int n = 0; n < batch_; ++n) {
            ...
            rowMap.device(context->eigen_gpu_device()) = colMap.shuffle(transposeIdx);
...
template<typename T>
class SimilarityInputGradKernelGPU : public SimilarityKernelCommon {
public:

    using Base = SimilarityKernelCommon;
    using Dtype = T;

    SimilarityInputGradKernelGPU(OpKernelConstruction *context) : Base(context) {}
    ...
    void Compute(OpKernelContext *context) override {
        ...
        input_grad_t.device(context->eigen_gpu_device()) = input_grad_t.constant(0);
        ...
        col_grad_buffer_t.device(context->eigen_gpu_device()) = col_grad_buffer_t.constant(0);
        ...
        for (int n = 0; n < batch_; ++n) {
            ...
            switch (similarity_function_) {
                case SIM_FUNC_L1:
                    ggemm_readc_gpu
                            ...
                            (K_, N_, M_, inter_params, top_diff, col_buff, col_diff_buff, make_vec2<Dtype>(0,0), 0, 0, 0);
                    break;
                case SIM_FUNC_L2:
                    if (normalization_term_) {
                        if (ignore_nan_input_) {
                            ggemm_readc_gpu
                                    ...
                                    (K_, N_, M_, inter_params, top_diff, col_buff, col_diff_buff,
            ...
            if (!is_1x1_) {
                simnets_tf::col2im_3d_gpu<Dtype>(
                        col_diff_buff,
                        channels_, height_, width_,
                        block_c_, block_h_, block_w_,
                        pad_c_, pad_h_, pad_w_,
...
REGISTER_KERNEL_BUILDER(
        ...
        SimilarityParametersGradKernelGPU<double>);
REGISTER_KERNEL_BUILDER(
        Name("SimilarityParametersGrad")
                .Device(DEVICE_GPU)
                .TypeConstraint<float>("T"),
        SimilarityParametersGradKernelGPU<float>);

REGISTER_KERNEL_BUILDER(
        Name("SimilarityInputGrad")
                .Device(DEVICE_GPU)
                .TypeConstraint<double>("T"),
        SimilarityInputGradKernelGPU<double>);
REGISTER_KERNEL_BUILDER(
        Name("SimilarityInputGrad")
                .Device(DEVICE_GPU)
                .TypeConstraint<float>("T"),
        SimilarityInputGradKernelGPU<float>);