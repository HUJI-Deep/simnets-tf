...
template<typename T>
class SimilarityKernelGPU : public SimilarityKernelCommon {
public:

    using Base = SimilarityKernelCommon;
    using Dtype = T;

    SimilarityKernelGPU(OpKernelConstruction *context) : Base(context) {}
    ...
    void Compute(OpKernelContext *context) override {
        ...
        interlace_gpu<Dtype>(params_size, templates_buff, weights_buff, inter_params);
        ...
        for (int n = 0; n < batch_; ++n) {
            ...
            switch (this->similarity_function_) {
                case SIM_FUNC_L1:
                    ggemm_gpu
                            ...
                            (M_, N_, K_, inter_params, col_buff, top_data + n * (out_c_ * out_h_ * out_w_),
                    ...
                    if (this->normalization_term_) {
                        if (this->ignore_nan_input_) {
                            ggemm_gpu
                                    ...
                                    (M_, N_, K_, inter_params, col_buff, top_data + n * (out_c_ * out_h_ * out_w_),
                                     make_vec2<Dtype>(0, Dtype(1) - normalization_term_fudge_), NAN, 0,
                            ...
                            ggemm_gpu
                                    ...
                                    (M_, N_, K_, inter_params, col_buff, top_data + n * (out_c_ * out_h_ * out_w_),
                                     make_vec2<Dtype>(0, Dtype(1) - normalization_term_fudge_), 0, 0,
            ...
            if (this->similarity_function_ == SIM_FUNC_L2 && this->normalization_term_ && !this->ignore_nan_input_) {
                output_t.device(context->eigen_gpu_device()) = output_t + output_t.constant(
                        Dtype(-0.5) * Dtype(K_) * std::log(2.0 * M_PI));
            }
        }
    }
};
...
REGISTER_KERNEL_BUILDER(
        ...
        SimilarityKernelGPU<float>);
REGISTER_KERNEL_BUILDER(
        Name("Similarity")
                .Device(DEVICE_GPU)
                .TypeConstraint<double>("T"),
        SimilarityKernelGPU<double>);
