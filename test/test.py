...
class SimilarityTests(tf.test.TestCase):
    ...
    def testSanity(self):
        with self.test_session():
            images = np.random.normal(size=(3,3,320,256)).astype(np.float32)
            #images = np.ones((1,1,3,3), np.float32)
            ...
            #templates = -np.ones((1,1,3,3), np.float32)
            templates = tf.constant(templates)

            sim = similarity(images, templates, weights, ksize=[1,3,3,1], strides=[1,1,1,1], padding='SAME')
            sim_ref = similarity_ref(images, templates, weights, ksize=[1,3,3,1], strides=[1,1,1,1], padding='SAME')
            s = sim.eval()
            sr = sim_ref.eval()
